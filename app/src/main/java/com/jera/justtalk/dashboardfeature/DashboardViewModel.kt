...
class DashboardViewModel(
    ...
    fun createThemeRecyclerOptions(collectionPath: String): Flow<FirestoreRecyclerOptions<ThemeModel>> {
        return flow {
            userStateFlow.value = ViewState.Loading
            firestoreRepository.createRecyclerOptions(collectionPath).collect { state ->
                when (state) {
                    is APIState.Success -> {
    ...
    fun createHighlightsRecyclerOptions(collectionPath: String): Flow<FirestoreRecyclerOptions<ThemeModel>> {
        return flow {
            userStateFlow.value = ViewState.Loading
            firestoreRepository.createHighlightRecyclerOptions(collectionPath).collect { state ->
                when (state) {
                    is APIState.Success -> {
                        userStateFlow.value = ViewState.Success
                        emit(state.data)
